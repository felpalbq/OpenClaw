...
def executor_cycle():
        ...
        if status in ("pending", "approved"):
            ...
            if not tool_def:
                ...
                results[result_id] = {
                    ...
                    "status": "error",
                    "data": None,
                    "error": f"tool_not_found: {action.get('tool')}",
                    "executed_at": _now(),
                    "execution_time_ms": 0,
                }
                changed = True
            ...
            changed = True
            write_state(state, agent="executor", reason="action_running")
    ...
    if changed:
        write_state(state, agent="executor", reason="cycle_completed")
...
def create_action(tool_name: str, params: dict = None,
    ...
    write_state(state, agent=source, reason=f"action_created_{tool_name}")
